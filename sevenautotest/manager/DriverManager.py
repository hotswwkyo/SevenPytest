...
class DriverManager(AttributeManager):
    ...
    def open_app(self, command_executor='http://127.0.0.1:4444/wd/hub', alias=None, 
                desired_capabilities=None, implicit_wait_timeout=0.0, browser_profile=None, proxy=None, keep_alive=True, direct_connection=False):
        app = appdriver.Remote(command_executor=command_executor,
                 desired_capabilities=desired_capabilities, browser_profile=browser_profile, proxy=proxy, keep_alive=keep_alive, direct_connection=direct_connection)
